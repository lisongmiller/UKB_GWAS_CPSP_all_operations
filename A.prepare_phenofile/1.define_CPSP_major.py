...
hr_opdate_dup = pd.read_csv('/home/miller/PhD_project/UKB_CPSP_allop/input/1.operation_demo_subset/popu_opdate_major_dup.csv.gz', sep=',', dtype=str)
hr_opdate = pd.concat([hr_opdate_uni,hr_opdate_dup]) # concatenate subjects with one operation and multiple operations

hr_opdate = hr_opdate.drop(['OPCS4','op_date'],axis='columns')
...
#######
os.chdir('/home/miller/PhD_project/UKB_CPSP_allop/input/4.phenotype_file/CPSP_phenotype/major')
...
QC_rm.to_csv('/home/miller/PhD_project/UKB_CPSP_allop/input/4.phenotype_file/CPSP_phenotype/major/qc_rm.txt',sep='\t',index=False)
...
cpsp_pheno_final.to_csv('./cpsp_phenotype_major.txt.gz',sep='\t',index=False)
...
cpsp_pheno_final_test2.to_csv('/home/miller/PhD_project/UKB_CPSP_allop/input/4.phenotype_file/CPSP_phenotype/major/cpsp_pheno_final_test2.txt.gz',sep='\t',index=False)

###Calculate for Meta
cpsp_pheno_final_test2_minor = pd.read_csv('/home/miller/PhD_project/UKB_CPSP_allop/input/4.phenotype_file/CPSP_phenotype/minor/cpsp_pheno_final_test2.txt.gz',sep='\t')
demo_meta = pd.concat([cpsp_pheno_final_test2,cpsp_pheno_final_test2_minor]) # concatenate subjects with one operation and multiple operations


#Opioid use
table = pd.crosstab(demo_meta['pheno'], demo_meta['opfl'])
print(table)
table_row_percentages = table.div(table.sum(axis=1), axis=0) * 100
# Print the row percentages of opioid use
print(table_row_percentages)
...
# Print opioid prescription numbers
grouped = demo_meta.groupby('pheno')
median = grouped['n_opioid'].median()
std_dev = grouped['n_opioid'].std()
